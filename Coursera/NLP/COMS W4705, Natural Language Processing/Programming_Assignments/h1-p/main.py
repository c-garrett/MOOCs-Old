...
class HMM(Object):
	...
	"Store the counts from a corpus. Takss a file handle as input"
	...
	def replace_words(self, sentence):
		...
		for pair in sentence:
			w, t = pair.split()
			new_sent.append(self.replace(w) + " " + t)
		return new_sent

	def argmax(ls):
		"Takes a list of pairs (item,score), return the argmax:"
	...
	def viterbi(hmm, sentence):
		...
		x = [""] + sentence
		y = [""] + (n + 1)
		def q(q, u, v): return hmm.trigram_prob((u, v, w))
		def e(x, u): return hmm.emission_prob(x, u)
		...
		# Follow the block pointers in the chart
