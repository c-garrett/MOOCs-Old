# Databricks notebook source exported at Sat, 20 Aug 2016 18:24:54 UTC
# MAGIC %md
...
powerPlantDF = sqlContext.read.format('csv').options(delimiter='\t', header = 'true', inferschema='true').load('dbfs:/databricks-datasets/power-plant/data')
...
altPowerPlantDF = sqlContext.read.format('csv').options(delimiter='\t', header = 'true').load('dbfs:/databricks-datasets/power-plant/data', schema = customSchema)
...
# MAGIC %sql
# MAGIC select V as ExhaustVacuum, PE as Power from power_plant
...
# MAGIC %sql
# MAGIC select AP as Pressure, PE as Power from power_plant
...
# MAGIC %sql
# MAGIC select RH as Humidity, PE as Power from power_plant
...
seed = 1800009193L
(split20DF, split80DF) = datasetDF.randomSplit([.2,.8],seed)
...
testingTransformed = cvModel.transform(testSetDF)
predictionsAndLabelsDF.selectExpr("PE", "Predicted_PE", "PE - Predicted_PE Residual_Error", "(PE - Predicted_PE) / {} Within_RSME".format(rmse)).registerTempTable("testingTransformed")
