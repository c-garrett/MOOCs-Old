# Databricks notebook source exported at Thu, 1 Sep 2016 16:00:31 UTC
# MAGIC %md
...
# TODO: Replace <FILL IN> with appropriate code
uniqueWords = wordPairs.distinct().count()
print uniqueWords
...
def wordCount(wordListRDD):
    ...
    """
    return wordListRDD.map(lambda x: (x,1)).groupByKey().map(lambda (x,y): (x,sum(y)))
...
def removePunctuation(text):
    ...
    """
    replacement = ""
    return re.sub("[^A-Za-z\s\d]",replacement,text).lower().strip()
...
# TODO: Replace <FILL IN> with appropriate code
shakespeareWordsRDD = shakespeareRDD.map(lambda x: x.split(' ')).flatMap(lambda x: x)
